...
const EtiquetaForm = ({ token, currentEtiqueta, setCurrentEtiqueta, fetchEtiquetas }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError(null); // Reiniciar el error antes de enviar

    try {
      if (currentEtiqueta) {
        // Editar etiqueta
        await axios.put(`http://localhost:8000/api/etiquetas/${currentEtiqueta.id}/`, 
          { name: name.toUpperCase() }, { headers: { Authorization: `Bearer ${token}` } });
      } else {
        // Crear nueva etiqueta
        await axios.post('http://localhost:8000/api/etiquetas/', 
          { name: name.toUpperCase() }, { headers: { Authorization: `Bearer ${token}` } });
      }
      setCurrentEtiqueta(null); // Limpiar la etiqueta actual
      setName(''); // Limpiar el campo de entrada
      fetchEtiquetas(); // Llamar a esta función para actualizar la lista de etiquetas
    } catch (err) {
      // Manejo de errores
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-light p-3 rounded shadow" style={{ maxWidth: '400px', margin: '0 auto' }}>
      ...
      {currentEtiqueta && (
        <button
          type="button"
          className="btn btn-secondary btn-block mt-2 btn-sm"
          onClick={() => setCurrentEtiqueta(null)}
        >
          Cancelar
        </button>
      )}
